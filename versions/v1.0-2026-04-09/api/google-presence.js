...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (ddgRes.ok) {
      ...
      // Abstract = Wikipedia or knowledge panel hit
      if (ddg.Abstract) {
        allResults.push({
          url:     ddg.AbstractURL || '',
  ...
  // ── Try Bing HTML scrape (more results, more detailed) ────────────────────
  try {
    ...
    if (bingRes.ok) {
      ...
      // Extract result count estimate
      const countMatch = html.match(/([0-9,]+)\s+results/i);
      if (countMatch) {
        const raw = parseInt(countMatch[1].replace(/,/g, ''), 10);
        allResults.push({ _resultCount: raw });
      }

      // Extract individual results: title, url, snippet
      ...
      resultBlocks.slice(0, 15).forEach(m => {
        ...
        if (url && !url.startsWith('javascript') && title) {
          allResults.push({ url, title, snippet, source: 'Bing' });
        }
      });

      // News results
      const newsBlocks = [...html.matchAll(/class="news-card[^>]*>[\s\S]*?<a[^>]+href="([^"]+)"[^>]*>([^<]+)<\/a>/g)];
      newsBlocks.slice(0, 5).forEach(m => {
        allResults.push({ url: m[1], title: m[2], snippet: '', source: 'Bing News' });
      });
    }
  ...
  // Score by source classification
  const found        = [];
  const dimensions   = { BA: 0, BD: 0, BR: 0 };
  ...
  // Normalise dimension boosts (cap at +15 per dimension)
  ...
  // Presence tier based on result count + finds
  let presenceTier;
  ...
  return res.json({
    ...
    data: {
      ...
      presenceTier,
      found,
      scoringBoosts: { BA: baBoost, BD: bdBoost, BR: brBoost },
